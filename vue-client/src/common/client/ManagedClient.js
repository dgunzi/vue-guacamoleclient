import ClipboardData from '../clipboard/ClipboardData'
...
import ClientIdentifier from '../navigation/ClientIdentifier'
import tunnelService from '../services/tunnelService'
import Guacamole from 'guacamole-common-js'
import preferenceService from '../settings/PreferenceService'
import guacAudio from './guacAudio'
import guacVideo from './guacVideo'
import guacImage from './guacImage'
 ...
 * @type Number
 */
let THUMBNAIL_UPDATE_FREQUENCY = 5000;
 ...
 *     ManagedClient.
 */
let ManagedClient = function ManagedClient(template) {
...
var getConnectString = function getConnectString(identifier, connectionParameters) {
    return new Promise(function(resolve, reject) {
        ...
        let optimalDpi = pixelDensity * 96;
        let optimalWidth = window.innerWidth * pixelDensity;
        let optimalHeight = window.innerHeight * pixelDensity;

        // Build base connect string
        let connectString =
            'token=' + encodeURIComponent(store.getters.authenticationResult.authToken) +
 ...
 */
let requestAudioStream = function requestAudioStream(client) {
    // Create new audio stream, associating it with an AudioRecorder
    let stream = client.createAudioStream(ManagedClient.AUDIO_INPUT_MIMETYPE);
    let recorder = Guacamole.AudioRecorder.getInstance(stream, ManagedClient.AUDIO_INPUT_MIMETYPE);
...
ManagedClient.getInstance = function getInstance(id, connectionParameters) {
    ...
    if (window.WebSocket) {
        tunnel = new Guacamole.ChainedTunnel(
                new Guacamole.WebSocketTunnel($axios.wsPath() + 'websocket-tunnel', true),
                new Guacamole.HTTPTunnel($axios.basePath() + 'tunnel', true)
        );
    } else { // If no WebSocket, then use HTTP.
        tunnel = new Guacamole.HTTPTunnel($axios.basePath() + 'tunnel', true);
    }
    ...
    let managedClient = new ManagedClient({
        id: id,
        client: client,
        tunnel: tunnel
    });
    ...
    client.onstatechange = (clientState) => {
            switch (clientState) {
                ...
                case 3:
                    ...
                    if (managedClient.clipboardData) { ManagedClient.setClipboard(managedClient, managedClient.clipboardData); }
    ...
    client.onclipboard = function clientClipboardReceived(stream, mimetype) {
        ...
        } else { // Otherwise read the clipboard data as a Blob
...
ManagedClient.setClipboard = function setClipboard(managedClient, data) {
    ...
    if (typeof data.data === 'string') {
        ...
        writer.sendEnd();
    } else { // Otherwise, assume the data is a File/Blob
...
ManagedClient.setArgument = function setArgument(managedClient, name, value) {
    ...
    if (managedArgument && ManagedArgument.setValue(managedArgument, value)) { delete managedClient.arguments[name]; }
};
...
ManagedClient.getArgumentModel = function getArgumentModel(client) {
    var model = {};
    client.arguments.forEach((managedArgument) => {
