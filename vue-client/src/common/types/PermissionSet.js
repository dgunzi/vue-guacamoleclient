 ...
 */
let containsPermission = function containsPermission(permMap, type) {
 ...
 *     true if the permission is present (granted), false otherwise.
 */
let hasPermission = function hasPermission(permMap, type, identifier) {
...
PermissionSet.removeSystemPermission = function removeSystemPermission(permSet, type) {
    ...
    let permLocation = permSet.systemPermissions.indexOf(type);
    if (permLocation !== -1) {
 ...
 *     already present in the given permission set.
 */
let addObjectPermission = function addObjectPermission(permMap, type, identifier) {
    // Pull array of permissions, creating it if necessary
    let permArray = permMap[identifier] = permMap[identifier] || [];
 ...
 *     present in the given permission set.
 */
let removeObjectPermission = function removeObjectPermission(permMap, type, identifier) {
    // Pull array of permissions
    let permArray = permMap[identifier];

    // If no permissions present at all, nothing to remove
    if (!(identifier in permMap)) { return false; }

    // Remove permission, if it exists
    let permLocation = permArray.indexOf(type);
    if (permLocation !== -1) {
